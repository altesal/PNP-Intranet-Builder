...
function Procesar-Navegacion {
    ...
    foreach ($item in $items) {
        ...
        if ($parentNode -eq $null) {
            $navNode = Add-PnPNavigationNode -Title $($descripcionItemMenu) -Url $($item.url) -Location QuickLaunch
            Write-Host "Nodo principal creado: $($descripcionItemMenu)"
        } else {
            $navNode = Add-PnPNavigationNode -Title $($descripcionItemMenu) -Url $($item.url) -Parent $parentNode.Id -Location QuickLaunch
